...
const Post = ({ post_value }) => {
  ...
  const toggleLikeButton = likeList?.filter((value) => {
    return (
      loginUser?.id === value?.user_id && post_value?.id === value?.Post_id
    );
  });
  ...
  return (
    <>
      <div className="post">
        ...
        {LikeCounting.length >= 2 ? (
          <p className="px-3">
            <strong>
              Liked by {LikeCounting[LikeCounting?.length - 1]?.User_namey} and{" "}
              {LikeCounting.length - 1} Others
            </strong>
          </p>
        ) : (
          <p className="px-3">
            <strong>{LikeCounting.length} Like </strong>
          </p>
        )}
            ...
            {toggleLikeButton.length == 0 ? (
              <span className="material-icons">
                <i
                  ...
                  onClick={() => LikePost(post_value?.id)}
                ></i>
              </span>
            ) : (
              <span className="material-icons">
                <i
                  style={{ color: "red" }}
                  onClick={() => deletePosts(toggleLikeButton[0]?.id)}
                  className="fas fa-heart"
                ></i>
              </span>
            )}
